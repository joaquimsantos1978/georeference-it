    ...
    <nav class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                <a href="{{ route('home') }}" class="text-xl font-bold text-green-600 dark:text-green-400 tracking-tight">
                    georeference.it
                </a>
                <div class="flex items-center gap-6">
                    <a href="{{ route('georef.index') }}" class="text-sm text-gray-600 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400">
                        {{ __('Georeference') }}
                    </a>
                    <a href="{{ route('leaderboard') }}" class="text-sm text-gray-600 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400">
                        {{ __('Leaderboard') }}
                    </a>
                    <a href="{{ route('about') }}" class="text-sm text-gray-600 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400">
                        {{ __('About') }}
                    </a>
                    @auth
                        {{-- Notifications bell --}}
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="relative p-1 text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400">
                        ...
                        {{-- User menu --}}
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400">
                                @if(auth()->user()->avatar)
                    ...
                    @else
                        ...
                        <a href="{{ route('register') }}" class="text-sm bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">{{ __('Register') }}</a>
                    @endauth
                </div>
            </div>
        </div>
