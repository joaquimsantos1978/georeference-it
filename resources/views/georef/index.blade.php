<x-layouts.georef>
            ...
            <div id="occ-section" class="p-3" style="flex:1;min-height:0;display:flex;flex-direction:column;overflow:hidden;">
                ...
                <div id="occ-select-controls" class="hidden mb-1" style="flex-shrink:0;">
                    <div class="flex flex-wrap gap-1 items-center">
                        ...
                        <select id="inst-filter" onchange="occSelectByInstitution(this.value)" class="text-xs border border-gray-200 rounded py-0.5 bg-white text-gray-600 max-w-full" style="max-width:150px;padding-left:6px;padding-right:25px;text-overflow:ellipsis;">
            ...
            <div style="pointer-events:auto;width:100%;max-width:420px;">
                <form id="nominatim-form" class="flex gap-1" onsubmit="event.preventDefault(); searchLocality(document.getElementById('nominatim-input').value.trim());">
                    <input type="text" id="nominatim-input" name="nominatim-search" autocomplete="on"
                        ...
                        placeholder="{{ __('Search place name to find coordinates...') }}">
        ...
        <div id="right-panel" style="width:320px; flex-shrink:0; z-index:10; display:flex; flex-direction:column; height:100%; overflow:hidden; border-left:1px solid #e5e7eb; position:relative;"
            ...
            <div id="similar-groups-wrap" style="display:none;flex-shrink:0;border-bottom:1px solid #d1d5db;">
                <div class="px-3 pt-2 pb-1" style="flex-shrink:0;">
                    <span class="text-xs font-medium text-gray-500 uppercase tracking-wide">{{ __('Similar locations') }}</span>
    ...
    const TXT = {
        ...
        agreed:       "{{ __('Agreed') }}",
        disagreed:    "{{ __('Disagreed') }}",
        noVotesYet:   "{{ __('No votes yet.') }}",
        loadingVotes: "{{ __('Loading votes...') }}",
...
if (isNaN(historyIndex) || historyIndex >= sessionHistory.length) historyIndex = sessionHistory.length - 1;
    ...
    function renderSystemSuggestionCard(r, i) {
        ...
        return '<div style="border:1px solid #bbf7d0;border-radius:6px;padding:6px 8px;background:#f0fdf4;margin-bottom:4px;">'
            ...
            + (sameAsExisting ? '' : '<button onclick="useSystemSuggestion('+i+')" class="use-similar-btn" style="font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;cursor:pointer;">Use this</button>')
            + '</div>'
            + '</div>'
            + '</div>';
    }
    function renderSystemSuggestionList(hasMore) {
        var html = _sysSuggResults.map(renderSystemSuggestionCard).join('');
        if (hasMore) {
...
function buildLocalityString(g) {
    ...
    const NOMINATIM_PAGE_SIZE = 15;
    let _nominatimQuery = '';
    // Nominatim doesn't report a total count — treat a full page as a signal there may
    // be more, and stop offering "load more" once a page comes back short.
    let _nominatimHasMore = false;
    ...
    function renderNominatimResultsList(selectedIndex) {
        ...
        if (_nominatimHasMore) {
            itemsHtml += '<button onclick="loadMoreNominatimResults()" style="width:100%;font-size:11px;padding:6px;border-radius:6px;border:1px solid #e5e7eb;color:#6b7280;background:white;cursor:pointer;margin-top:2px;">'+TXT.loadMore+'</button>';
        }
    ...
    function addNominatimMarkers(results, startIndex) {
        (window._nominatimResultMarkers || []).push(...results.map((r, i) => {
            var lat = parseFloat(r.lat), lon = parseFloat(r.lon);
            return L.circleMarker([lat, lon], {radius: 6, color: '#16a34a', weight: 2, fillColor: '#fff', fillOpacity: 0.9})
                .bindTooltip(r.display_name, {direction: 'top'})
                .on('click', () => applyNominatimResult(startIndex + i))
                .addTo(map);
        }));
    }
    ...
    async function searchNominatim(query) {
        ...
        showLocalitySearchResults();
        try {
            const results = await (await fetch('https://nominatim.openstreetmap.org/search?q='+encodeURIComponent(query)+'&format=json&polygon_geojson=1&limit='+NOMINATIM_PAGE_SIZE, { headers: {'Accept-Language':'en'} })).json();
            if (token !== _localitySearchToken) return; // a newer search/close superseded this one
            ...
            _nominatimHasMore = results.length >= NOMINATIM_PAGE_SIZE;
            // Drop a marker for every hit so they can be compared on the map at a glance,
            // not just one at a time.
            addNominatimMarkers(results, 0);
    ...
    async function loadMoreNominatimResults() {
        if (!_nominatimQuery) return;
        const token = _localitySearchToken;
        const excludeIds = (window._nominatimResults || []).map(r => r.place_id).filter(Boolean).join(',');
        try {
            const results = await (await fetch('https://nominatim.openstreetmap.org/search?q='+encodeURIComponent(_nominatimQuery)+'&format=json&polygon_geojson=1&limit='+NOMINATIM_PAGE_SIZE+(excludeIds ? '&exclude_place_ids='+excludeIds : ''), { headers: {'Accept-Language':'en'} })).json();
            if (token !== _localitySearchToken) return;
            _nominatimHasMore = results.length >= NOMINATIM_PAGE_SIZE;
            if (results.length) {
                addNominatimMarkers(results, (window._nominatimResults || []).length);
                window._nominatimResults = (window._nominatimResults || []).concat(results);
            }
            renderNominatimResultsList(null);
        } catch(e) { /* keep existing list on failure */ }
    }
    function applyNominatimResult(index) {
        ...
        renderNominatimResultsList(index);
    }
...
function updateHistoryNav() {
        ...
        list.innerHTML = groups.map(function(g) {
            ...
            const suggHtml = (g.suggestions && g.suggestions.length) ? g.suggestions.map(function(s) {
                ...
                return '<div class="sugg-card" style="font-size:11px;border-radius:6px;padding:8px;margin-top:4px;">'
                    ...
                    + '</div>'
                    + '<div style="display:flex;justify-content:flex-end;margin-top:4px;">'
                    + '<button onclick="openGroupOccPopup('+g.id+','+g.occurrence_count+')" style="font-size:10px;color:#3b82f6;background:none;border:none;cursor:pointer;padding:0;">'+g.occurrence_count+' {{ __("occurrences") }} · {{ __("see list") }} ↗</button>'
                    + '</div>'
                    ...
                    + '</div>'
                    + '<div onclick="showVotesPopup('+s.id+',event)" style="cursor:pointer;position:relative;height:6px;border-radius:4px;margin-top:6px;background:linear-gradient(to right,#b91c1c,#f59e0b,#15803d);">'
                    ...
                    + '</div>'
                    + '</div>';
            }).join('') : '<div style="display:flex;justify-content:flex-end;margin-top:4px;">'
                + '<button onclick="openGroupOccPopup('+g.id+','+g.occurrence_count+')" style="font-size:10px;color:#3b82f6;background:none;border:none;cursor:pointer;padding:0;">'+g.occurrence_count+' {{ __("occurrences") }} · {{ __("see list") }} ↗</button>'
                + '</div>';
    ...
    async function showVotesPopup(suggestionId, event) {
        ...
        popup.textContent = TXT.loadingVotes;
        document.body.appendChild(popup);
        ...
        setTimeout(function(){ document.addEventListener('click', close); }, 0);
        try {
            const data = await (await fetch(APP_URL+'/georef/suggestion/'+suggestionId+'/votes')).json();
            if (!document.body.contains(popup)) return; // closed before it loaded
            var fmt = function(list) { return list.map(function(v){ return escHtml(v.name)+' ('+v.points+(v.points===1?' pt':' pts')+')'; }).join(', '); };
            var html = '';
            if (data.agree.length) html += '<div><strong style="color:#15803d">'+TXT.agreed+':</strong> '+fmt(data.agree)+'</div>';
            if (data.disagree.length) html += '<div style="margin-top:4px"><strong style="color:#b91c1c">'+TXT.disagreed+':</strong> '+fmt(data.disagree)+'</div>';
            popup.innerHTML = html || '<span style="color:#9ca3af">'+TXT.noVotesYet+'</span>';
        } catch(e) { popup.textContent = TXT.searchFailed; }
    }
    ...
    function renderGroup(group, occurrences, ungeorefTotal, georefOccurrences, suggestions, comments, similarGroups, notGeoreferenceableCount) {
        ...
        if (allGeoref) {
            ...
            georefOccurrences.forEach(function(o) {
                ...
                gbifHtml += '<div class="sugg-card" style="font-size:11px;border-radius:6px;padding:8px;margin-bottom:4px">' +
                    ...
                    '</div>' +
                    '<div style="display:flex;justify-content:flex-end;margin-top:4px;">' +
                    '<button onclick="openGbifOccPopup('+JSON.stringify(ids)+')" style="font-size:10px;color:#3b82f6;background:none;border:none;cursor:pointer;padding:0">'+cnt+' {{ __("occ.") }} · {{ __("see list") }} ↗</button>' +
                    '</div>' +
                    '<div style="display:flex;justify-content:space-between;margin-top:4px;color:#9ca3af">' +
        ...
        if (suggestions&&suggestions.length>0) {
            ...
            suggestions.forEach(function(s,i){
                ...
                var seeListRow = s.cluster_count > 0
                    ? '<div style="display:flex;justify-content:flex-end;margin-top:4px;">'+
                      '<button onclick="openOccPopup('+s.id+','+s.cluster_count+','+JSON.stringify(s.cluster_occurrence_ids)+')" style="font-size:10px;color:#3b82f6;background:none;border:none;cursor:pointer;padding:0;">'+s.cluster_count+' {{ __("occurrences") }} · {{ __("see list") }} ↗</button>'+
                      '</div>'
                    : '';
